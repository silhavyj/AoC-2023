...
int64_t Calculate_Prediction(const std::vector<int64_t>& history)
{
    ...
    while (!all_zeros)
    {
        ...
        last_values.emplace_back(new_differences.back());

        last_differences = new_differences;
    }

    std::vector<int64_t> predictions(last_values.size(), 0);

    for (int64_t i = predictions.size() - 2; i >= 0; --i)
    {
        predictions[i] = last_values[i] + predictions[i + 1];
    }

    const int64_t prediction = history[history.size() - 1] + predictions[0];
